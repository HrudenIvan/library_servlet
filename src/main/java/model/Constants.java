...
public class Constants {
	...
	public static final String GET_ALL_BOOKS =
			...
			+ "JOIN author a "
			+ "JOIN publisher p "
			+ "WHERE b.author_id = a.id AND b.publisher_id = p.id AND b.available >0";
	...
	public static final String GET_BOOKS_BY_TITLE = 
			"SELECT b.id AS bid, b.title, b.available, b.total, a.id AS aid, a.first_name, a.last_name, p.id AS pid, p.name, b.release_date "
			+ "FROM book b "
			+ "JOIN author a "
			+ "JOIN publisher p "
			+ "WHERE b.available > 0 AND b.author_id = a.id AND b.publisher_id = p.id AND b.title LIKE ?";
